...
describe("Teste - Login", () => {
  ...
  context("Treinamento sem aprovação gestor na turma", () => {
    ...
    it.only("Criando um treinamento", () => {
      ...
      //teste
      cy.get('#cke_660_contents > .cke_wysiwyg_frame', { timeout: 15000 }) // Aumenta o tempo limite
      .should('be.visible') // Garante que está visível
      .then(($iframe) => {
      cy.wrap($iframe.contents().find('body')) // Envolve o conteúdo dentro do iframe
        .should('exist') // Garante que existe
        .click() // Clica para ativar
        .type('Texto dentro do CKEditor via Cypress!'); // Insere texto
    });
