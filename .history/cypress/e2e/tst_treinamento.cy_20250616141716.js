...
describe("Teste - Login", () => {
  ...
  context("Treinamento sem aprovação gestor na turma", () => {
    ...
    it.only("Criando um treinamento", () => {
      ...
      //teste
      cy.get('#cke_660_contents > .cke_wysiwyg_frame') // Localiza o iframe
      .its('0.contentDocument') // Obtém o documento interno do iframe
      .should('exist') // Confirma que ele existe
      .then((doc) => {
        cy.wrap(doc.body).as('editorBody'); // Envolve o body do editor
      });

      cy.get('@editorBody')
        .click() // Clica dentro do editor para ativá-lo
        .type('Texto inserido no CKEditor via Cypress!'); // Insere o texto
