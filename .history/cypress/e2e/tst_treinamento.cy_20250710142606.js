...
describe("Teste - Login", () => {
  ...
  context("Treinamento com aprovação gestor na turma", () => {
    ...
    it.only("Criando um treinamento", () => {
      ...
      //Autores
      cy.get('[authors=""] > .ng-isolate-scope > .multiselect > .border > div > .ui-select-search').type("mik"); // Digitar nome do autor
      cy.wait(3000); // Espera para o modal abrir
      cy.get('.content-box-footer > .flex > .btn-swipe-accent').click(); // Botão de salvar autores

      cy.get('.multiselect.ng-dirty > .btn').click(); // Botão de pesquisar autor
      cy.wait(10000); // Espera para o modal abrir
